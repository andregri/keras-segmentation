...
def color_gt_image(gt_img, id2color):
    """Take a ground-truth image where each pixel's value corresponds to a
    class ID (1-channel image) and return a RGB image (3-channels) where each
    class has a different color. label2color maps from class to color.
...
def IoU(true_label_mask, pred_label_mask, id2name):
    ...
    Arguments:
        ...
        id2name (dict): a dictionary whose keys are class IDs and values are
            the class names.
    """
